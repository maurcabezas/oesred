...
# 	###### move files
	...
	!awk '{for(i=2;i<=NF;i++) printf $i" ";print ",mode=\047h\047)"}' tmp.list | awk '{print ",idtarget="$0}' > tmp3.list
...
if (zerocorcal==yes){
	...
	ccdproc.zero="../zero/Zero.fit"
		if (iftrimc==yes){
		ccdproc (images="T@flat.dat",output="ZT@flat.dat")
		} else {
	...
	ccdproc.zero="../zero/Zero.fit"
			if (iftrimc==yes){
		ccdproc (images="T@comp.dat",output="ZT@comp.dat")
		} else {
...
#	imcombine (input="@comp.dat",output = "comp.fits")
		if (iftrimc==yes){
		imcombine (input="ZT@comp.dat",output = "../ZTcomp.fit")
		} else {
...
if (flatcomb==yes){
	cd "flat/"
		if (iftrimc==yes){
		imcombine (input="ZT@flat.dat",output = "../ZTflat.fit")
		} else {
...
# start iterations
 ...
 while(!stop) {
  ...
  convolve(firstsel,gfirstsel,gkernel,xkernel=1,ykernel=1)
  ...
  convolve(gfirstsel,finalsel,gkernel,xkernel=1,ykernel=1)
...
if (objectapall==yes){
	...
	##
	cd (oname)
	apall (input=inobject, output="A"//inobject,mode='h')
	cd "../"
}
...
if (heliocor==yes){
	...
	setjd (images="FA"//inobject,mode='h')
	rv
	unlearn rvcorrect
